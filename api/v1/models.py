...
class HouseFeaturesRequest(BaseModel):
    ...
    sqft_living: int = Field(
        ..., ge=100, le=20000, description="Square feet of living space"
    )
    ...
    waterfront: Optional[int] = Field(
        0, ge=0, le=1, description="Waterfront property (0 or 1)"
    )
    view: Optional[int] = Field(0, ge=0, le=4, description="View rating (0-4)")
    condition: Optional[int] = Field(
        3, ge=1, le=5, description="Condition rating (1-5)"
    )
    grade: Optional[int] = Field(7, ge=1, le=13, description="Grade rating (1-13)")
    sqft_above: int = Field(
        ..., ge=100, le=20000, description="Square feet above ground"
    )
    sqft_basement: Optional[int] = Field(
        0, ge=0, le=10000, description="Square feet of basement"
    )
    yr_built: int = Field(..., ge=1900, le=2025, description="Year built")
    yr_renovated: Optional[int] = Field(
        0, ge=0, le=2025, description="Year renovated (0 if never)"
    )
    ...
    sqft_living15: Optional[int] = Field(
        None, ge=100, le=20000, description="Living space of 15 nearest neighbors"
    )
...
class PredictionResponse(BaseModel):
    ...
    prediction_type: Optional[str] = Field(
        None, description="Type of prediction (e.g., minimal_features)"
    )
    core_features_used: Optional[int] = Field(
...
class ModelInfoResponse(BaseModel):
    ...
    total_features: int = Field(
        ..., description="Total number of features the model uses"
    )
    features: List[str] = Field(..., description="List of feature names")
    demographics_zipcodes: int = Field(
