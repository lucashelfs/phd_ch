...
from .exceptions import DemographicDataError, ModelLoadError, PredictionError
...
class HousePricePredictor:
    ...
    def _enrich_with_demographics(
        self, house_data: Dict[str, Union[str, float, int]]
        ...
        if demographic_row.empty:
            raise DemographicDataError(
                f"No demographic data found for zipcode: {zipcode}", zipcode=zipcode
            )
    ...
    def _prepare_features(
        self, enriched_data: Dict[str, Union[str, float, int]]
    ) -> pd.DataFrame:
        """
