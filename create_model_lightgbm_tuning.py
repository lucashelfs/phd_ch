...
def train_and_evaluate_model(
    ...
    run_name = f"LightGBM_Tuning_{combination_idx + 1:02d}_n{model_params['n_estimators']}_d{model_params['max_depth']}_lr{model_params['learning_rate']}"
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        mlflow.set_tags(
            {
                "model_type": "LGBMRegressor_Tuned",
...
def main():
    """Load data, perform hyperparameter tuning for LightGBM with MLflow tracking."""

    # Set MLflow tracking URI and experiment
    mlflow.set_tracking_uri(TRACKING_URI)
    mlflow.set_experiment(EXPERIMENT_NAME)

    # Enable autologging for LightGBM (but we'll do manual logging for better control)
    ...
    if results:
        ...
        for i, result in enumerate(results_sorted[:5]):
            ...
            print(
                ...
                f"leaves={result['params']['num_leaves']}"
            )
        ...
        # Save summary
        summary = {
            "tuning_type": "LightGBM Hyperparameter Tuning",
        ...
        with open("lightgbm_tuning_summary.json", "w") as f:
            json.dump(summary, f, indent=2, default=str)

        print(f"\nSummary saved to: lightgbm_tuning_summary.json")
